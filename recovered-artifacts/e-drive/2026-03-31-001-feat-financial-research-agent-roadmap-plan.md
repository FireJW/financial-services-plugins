...
- a minimal repo-local shell now exists under `apps/financial-research-cli/`
- current surfaces: `article-publish`, `article-publish-reuse`, `morning-note`, `earnings-update`, `operator-summary`, `theme-screen`, `x-index`
- current package acceptance:
  - `cmd /c npm --prefix apps/financial-research-cli run acceptance`
  - `cmd /c npm --prefix apps/financial-research-cli run acceptance:offline`
- this lands the thin-shell boundary, but not the full standalone-product story
  or broader workflow/menu expansion
