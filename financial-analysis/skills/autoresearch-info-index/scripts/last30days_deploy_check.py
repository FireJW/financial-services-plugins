#!/usr/bin/env python3
"""
Run a local deployment check for a separate last30days installation.
"""
...
from last30days_deploy_check_runtime import load_json, run_last30days_deploy_check, write_json


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check whether a separate last30days deployment is present and usable."
    )
    parser.add_argument("input", nargs="?", help="Optional path to a deploy-check request JSON file")
    parser.add_argument("--install-root", help="Override the last30days install root to inspect")
...
def main() -> None:
    ...
    try:
        ...
        result = run_last30days_deploy_check(payload)
        if not args.quiet:
