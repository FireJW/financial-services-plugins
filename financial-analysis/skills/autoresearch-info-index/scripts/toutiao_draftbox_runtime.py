#!/usr/bin/env python3
"""Push a toutiao-fast-card-package/v1 to Toutiao via CDP browser automation.

Phase 1: browser_session only (no API).
Phase 2 (future): API-first with browser fallback, same as WeChat.
"""
...
def build_toutiao_browser_manifest(fast_card_package: dict[str, Any], workdir: Path) -> tuple[Path, dict[str, Any]]:
    browser_dir = workdir / ".tmp" / "toutiao-browser-session-push"
...
def push_fast_card_to_toutiao(
    ...
    *,
    ...
    """Push a Fast Card package to Toutiao.

    Phase 1: browser_session only.
    """
    request = safe_dict(raw_request)
    fast_card_package = safe_dict(request.get("fast_card_package"))
    ...
    # Phase 1: always use browser session (no API available yet)
    ...
    # Phase 2 placeholder: API path (not yet implemented)
    return {
        "status": "error",
        "push_backend": push_backend,
        "review_gate": review_gate,
        "error_message": f"Push backend '{push_backend}' is not yet supported for Toutiao. Use 'browser_session' or 'auto'.",
    }
