#!/usr/bin/env python3
"""
Run a one-shot recency-first news index retrieval.
"""
...
from news_index_runtime import load_json, run_news_index, write_json


def parse_args() -> argparse.Namespace:
    default_input = SCRIPT_DIR.parent / "examples" / "news-index-crisis-request.json"
    parser = argparse.ArgumentParser(
        description="Run a recency-first news index request and emit structured output."
    )
    parser.add_argument(
        "input",
        nargs="?",
        default=str(default_input),
        help="Path to a retrieval_request JSON file",
    )
