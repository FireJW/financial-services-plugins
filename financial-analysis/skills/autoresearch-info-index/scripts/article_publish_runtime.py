...
def select_cover_plan(image_assets: list[dict[str, Any]], cover_candidates: list[dict[str, Any]], request: dict[str, Any]) -> dict[str, Any]:
    ...
    dedicated_candidates = [item for item in cover_candidates if not item.get("from_selected_images")]
    screenshot_candidates = [item for item in cover_candidates if "screenshot" in clean_text(item.get("role")).lower()]
    body_candidates = [item for item in cover_candidates if item.get("from_selected_images")]
    ...
    if dedicated_candidates:
        ...
        candidate = sorted(body_candidates, key=lambda item: int(item.get("body_order", 999)))[0]
