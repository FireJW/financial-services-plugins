#!/usr/bin/env python3
"""
Run a one-shot last30days import and bridge it into news-index.
"""
...
from last30days_bridge_runtime import load_json, run_last30days_bridge, write_json


def parse_args() -> argparse.Namespace:
    default_input = SCRIPT_DIR.parent / "examples" / "last30days-bridge-input.json"
    parser = argparse.ArgumentParser(
        description="Bridge a last30days-style result into the recency-first news-index workflow."
    )
    parser.add_argument(
        "input",
        nargs="?",
        default=str(default_input),
        help="Path to a last30days bridge request JSON file",
    )
...
def main() -> None:
    ...
    try:
        ...
        result = run_last30days_bridge(payload)
        if not args.quiet:
