...
from benchmark_library_refresh_runtime import load_json, write_json
from benchmark_readiness_runtime import run_benchmark_readiness_audit


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit whether a benchmark refresh request is actually ready for safe daily execution."
    )
...
def main() -> None:
    ...
    try:
        ...
        result = run_benchmark_readiness_audit(payload)
        if not args.quiet:
