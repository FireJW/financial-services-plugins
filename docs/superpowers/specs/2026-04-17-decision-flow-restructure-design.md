...
Introduce a new primary summary section `## 决策流` that consolidates the
headline role currently split across `Event Board` and `Chain Map`, while
keeping `Decision Factors` in markdown for now as the richer detailed layer.
Each ticker gets one self-contained decision card. `Event Cards` section stays
as-is for detailed evidence archival.

## 2. Approach: Gradual Merge (方案 C)

- **Markdown:** New `## 决策流` becomes the main summary layer. In phase 1 it
  replaces `Event Board` and absorbs the actionable part of `Chain Map`, but it
  does **not** remove `Decision Factors` yet.
...
1. Title line: `### {ticker} | {action} | {score}分 | {trading_profile_bucket}`
...
- Start from `trading_profile_judgment`, then append explicit score / threshold /
  gap information from the current result payload instead of inventing a new
  `logic_summary` field.
...
- **链条:** Chain name, trading-profile bucket, chain playbook sentence.
...
- Start from `trading_profile_usage`, then append one concise watchpoint sentence
  derived from existing structured fields already present in the enriched
  candidate / decision-factor data. Do **not** introduce a new
  `next_watch_items` dependency in phase 1.
...
| Decision Factors | Decision Factors (unchanged in phase 1) |
| 直接可执行 / 重点观察 / 链条跟踪 | (unchanged) |
| Event Board | **REPLACED by `## 决策流`** |
| Chain Map | **REMOVED as standalone section; its actionable content is folded into `## 决策流`** |
...
- `earnings_momentum_discovery.py` — optional helper additions are allowed if
  they keep judgment / usage / trigger generation cleaner
- `tests/test_month_end_shortlist_degraded_reporting.py` — primary markdown
  assertions for decision flow layout
- `tests/test_earnings_momentum_discovery.py` — helper-level assertions if new
  trigger / summary helpers are added
...
- Unit tests for card generation should live in
  `tests/test_month_end_shortlist_degraded_reporting.py` and, if helper
  extraction is used, `tests/test_earnings_momentum_discovery.py`

### Slice 2: Build decision flow markdown
- New function `build_decision_flow_markdown()` — render cards to markdown
- Replace `Event Board` and absorb the user-facing summary role of `Chain Map`
- Keep `Decision Factors` in markdown during phase 1
- BOM encoding tests
...
- Removing `Decision Factors` from markdown in phase 1
