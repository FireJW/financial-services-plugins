# -*- coding: utf-8 -*-
'''
Antai Science and Technology (000969.SZ) DCF Valuation Model
Advanced Metal New Materials Platform
...
# ============================================================
TICKER = "000969.SZ"
NAME = "Antai Science & Technology"
DATE = "2026-03-11"

PRICE = 24.03
SHARES = 1051       # M shares
MCAP = PRICE * SHARES

hist = {
    "year":    [2021,    2022,     2023,     2024],
    "rev":     [6271,    7406,     8187,     7573],
    "ebit":    [420,     510,      620,      907],
    "ebit_pct":[0.067,   0.069,    0.076,    0.120],
    "tax":     [70,      85,       100,      150],
    "ni":      [175,     214,      254,      372],
    "da":      [240,     260,      307,      275],
    "capex":   [280,     320,      190,      401],
    "ebitda":  [660,     770,      927,      1182],
}

# Balance Sheet
DEBT = 750          # M estimated interest-bearing debt
CASH = 2012         # M cash (end of 2024)
NET_DEBT = DEBT - CASH  # Net cash position ~-1262

# WACC Inputs
Rf = 0.022
BETA = 1.21         # Higher beta - specialty materials, cyclical
ERP = 0.065
KE = Rf + BETA * ERP
KD_PRE = 0.040
TAX_R = 0.15        # High-tech enterprise preferential rate
KD_POST = KD_PRE * (1 - TAX_R)
EV = MCAP + max(NET_DEBT, 0)  # Net cash, so EV = MCap essentially
EQ_W = MCAP / (MCAP + max(NET_DEBT, 0) + 1)  # almost 100% equity
DT_W = 1 - EQ_W
WACC_V = KE * EQ_W + KD_POST * DT_W

proj = {
    "years": [2025, 2026, 2027, 2028, 2029],
    "bear":  {"rg": [0.03, 0.03, 0.02, 0.02, 0.02], "em": [0.09, 0.09, 0.08, 0.08, 0.08]},
    "base":  {"rg": [0.06, 0.08, 0.07, 0.06, 0.05], "em": [0.12, 0.13, 0.13, 0.12, 0.12]},
    "bull":  {"rg": [0.10, 0.12, 0.10, 0.08, 0.06], "em": [0.15, 0.16, 0.16, 0.15, 0.14]},
}
DA_P = 0.035        # D&A % of revenue
CX_P = 0.045        # CapEx % of revenue
NW_P = 0.015        # NWC change % of delta revenue
TG = {"bear": 0.02, "base": 0.03, "bull": 0.04}
...
R = 1
sc(ws, R, 1, f"Antai S&T (000969.SZ) DCF Model", TF)
...
mkt = [
    ...
    ("Shares Outstanding (M)", SHARES, N, "Source: 10.51 yi"),
    ("Market Cap (M)", None, N, None),
    ("Total Debt (M)", DEBT, N, "Short+Long term borrowings"),
...
sc(ws, 38, 1, "D&A % Rev", SF, GY); sc(ws, 38, 2, DA_P, BF, LG, P, "~3.5% normalized")
sc(ws, 39, 1, "CapEx % Rev", SF, GY); sc(ws, 39, 2, CX_P, BF, LG, P, "~4.5% avg")
...
tgv = [0.015, 0.020, 0.025, 0.030, 0.040]
...
sc(ws2, 4, 1, "Beta (5Y monthly vs CSI300)", KF); sc(ws2, 4, 2, BETA, BF, LG, '0.00', "High beta: cyclical materials")
...
segs = [
    ("Advanced Functional Materials", 2920, 0.386, "Rare earth NdFeB + amorphous"),
    ("Specialty Powder Metallurgy", 2780, 0.367, "Tungsten-Mo + superhard + soft mag"),
    ("High-Quality Special Steel", 1873, 0.247, "Welding materials + high-speed steel"),
]
for i, (nm, rv, pct, dr) in enumerate(segs):
    r = 4 + i
    sc(ws3, r, 1, nm, SF, GY); sc(ws3, r, 2, rv, BF, fm=N, cm="FY2024 est based on H1 ratios")
    sc(ws3, r, 3, pct, KF, fm=P); sc(ws3, r, 4, dr, KF)

sc(ws3, 7, 1, "Total Revenue", SF, YL); sc(ws3, 7, 2, "=SUM(B4:B6)", RF, YL, N)
sc(ws3, 7, 3, "=SUM(C4:C6)", RF, YL, P)

sc(ws3, 9, 1, "KEY NOTES:", SF)
sc(ws3, 10, 1, "1. Rare earth NdFeB permanent magnets: top global supplier, 12000t capacity", KF)
sc(ws3, 11, 1, "2. Soft magnetic powder: largest domestic supplier, AI/NEV applications", KF)
sc(ws3, 12, 1, "3. Parent company China Iron & Steel Research Institute Group (CISRI)", KF)
sc(ws3, 13, 1, "4. ROE improving: 3.7%(2021)->4.3%(2022)->5.0%(2023)->6.7%(2024)", KF)
sc(ws3, 14, 1, "5. Net cash position (~1.26B) provides safety margin", KF)
sc(ws3, 15, 1, "6. Growth drivers: NEV, robots, AI servers, aerospace", KF)

# ============================================================
# SAVE
# ============================================================
out = r"c:\Users\rickylu\.gemini\antigravity\scratch\financial-services-plugins\000969_DCF_Model_2026-03-11.xlsx"
wb.save(out)
